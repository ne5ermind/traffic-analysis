...
4. ByteTrack с Kalman filter сохраняет ID при короткой потере объекта.
   Порог создания и продолжения трека зависит от режима: «Точный» принимает
   менее уверенные детекции маленьких машин и дольше ждёт их после перекрытия.
...
6. Консервативное соединение коротких фрагментов: один кандидат, совместимый
   тип объекта, разрыв до 2,5 секунды, допустимая позиция с учётом размера
   объекта и совпадающее направление движения. Это помогает после столбов,
   деревьев и кратких провалов детектора. Одновременные треки, соседние полосы
   и длительный выход/возврат не объединяются.
...
8. Нужно минимум три похожих трека для нового автоматического направления.
   Неподвижные детекции и дрожание рамки исключаются из итогов и отдельно
   показываются в диагностике. Изолированные, слишком короткие по пути или
   отличающиеся траектории остаются `unknown`; географические названия не
   генерируются.
...
```sh
python scripts/http_smoke.py --video "/path/to/intersection.mov" --profile accurate --timeout 3600
```
...
python scripts/real_model_smoke.py --video "/path/to/intersection.mp4" --profile accurate --output data/real-smoke
```

Для тёмного, размытого видео, далёких машин или частых перекрытий выбирайте
режим «Точный». Он обрабатывает каждый кадр с размером входа 960 px и применяет
более чувствительные параметры детектора и трекера, поэтому работает заметно
дольше. Режим не восстанавливает детали, которых нет в исходной записи:
сверяйте результат с картограммой, предупреждениями качества и ручным эталоном.
